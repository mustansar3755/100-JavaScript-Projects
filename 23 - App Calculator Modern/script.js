...
function calculateAge() {
  ...
  const now = todayInput ? new Date(todayInput + "T00:00:00") : new Date();
  ...
  const daysLeft = Math.max(
    0,
    Math.round((startOfDayUTC(nextBD) - startOfDayUTC(todayLocal)) / 86400000)
  );
  ...
  byId("daysLeft").textContent = `${daysLeft} day${daysLeft === 1 ? "" : "s"}`;
